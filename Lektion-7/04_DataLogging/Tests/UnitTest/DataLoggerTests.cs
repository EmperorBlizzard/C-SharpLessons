...
{
    [Fact]
    public void Messages_Should_WhenFileDoesNotExistReturnNull()
    {
        //Arrange
        var mockFileHandler = new Mock<IFileHandler>();
        mockFileHandler.Setup(fileHandler => fileHandler.ReadFromFile(It.IsAny<string>())).Returns((IEnumerable<string>) null!);
